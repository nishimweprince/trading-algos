...
def run_s1_target_hit(
    ...
    *,
    ...
    reference = EngineParams.model_validate(
        params.model_dump() | {"entry_mode": S1_REFERENCE_MODE}
    )
    ...
    edges = tercile_edges(
        [value for value in atr_by_structure.values() if value is not None]
    )
        ...
        for horizon in S1_HORIZON_HOURS:
            ...
            for k in S1_K_VALUES:
                ...
                survived = sum(
                    1
                    for item in subset
                    if item.mfe_r_before_lock_by_horizon.get(key, 0.0) >= k
                )
                cells.append(
                    S1ReachCell(
...
def _excursions(structures: list[S1Structure]) -> list[S1ExcursionCell]:
    ...
    for group_kind, group_key, subset in _groups(structures):
        for horizon in S1_HORIZON_HOURS:
            ...
            mfe_pips = [
                value * item.s_pips for value, item in zip(mfe_r, subset, strict=True)
            ]
            mae_pips = [
                value * item.s_pips for value, item in zip(mae_r, subset, strict=True)
            ]
            mfe_orb = [
    ...
    lines += markdown.identity_section(
        ...
        extra=[
            ...
            (
                ...
                ", ".join(markdown.num(edge) for edge in report.atr_tercile_edges_pips)
                or "—",
            ),
        ],
    )
...
def _reach_section(report: S1TargetHitReport) -> list[str]:
    ...
    return [
        ...
        [
            "Group", "Key", "Horizon h", "k", "n", "Reached", "Rate", "CI low", "CI high",
            "Reached before lock", "Rate", "CI low", "CI high",
        ],
        rows,
        align_right_from=2,
    )
...
def _excursion_section(report: S1TargetHitReport) -> list[str]:
    ...
    return [
        ...
        [
            "Group", "Key", "Horizon h", "n",
            "MFE pips p50", "MFE pips p95", "MFE pips mean",
            "MAE pips p50", "MAE pips p95", "MAE pips mean",
            "MFE R p50", "MFE R p95", "MAE R p50", "MAE R p95",
            "MFE ORB p50", "MFE ORB p95", "MAE ORB p50", "MAE ORB p95",
        ],
        rows,
        align_right_from=2,
    )
...
def _structure_section(report: S1TargetHitReport) -> list[str]:
    ...
    return [
        ...
        [
            "Pair", "Session", "Survivor", "First stop", "S pips", "ORB pips", "ATR pips",
            "ATR tercile", "Lock price", "Lock touched", "Fwd bars",
            f"MFE R {longest}", f"MAE R {longest}", f"MFE R before lock {longest}",
            "Realised outcome",
        ],
        rows,
        align_right_from=3,
