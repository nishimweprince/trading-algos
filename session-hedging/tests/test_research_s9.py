...
from research.s9_regime import (
...
def test_every_mode_is_split_the_same_way(s9) -> None:
    ...
    for mode in S9_MODES:
        splits = {
            (cell.split_kind, cell.split_key)
        ...
        assert {
            ("trend_regime", regime) for regime in ("up", "down", "flat", "warmup")
        } <= splits
...
def test_splits_partition_the_completed_structures(s9) -> None:
    for mode in S9_MODES:
        ...
        for kind in ("calendar_half", "trend_regime", "session"):
            ...
            assert sum(cell.gross_pips for cell in subset) == pytest.approx(
                overall.gross_pips
            )
...
def test_flags_only_fire_above_the_threshold(s9) -> None:
    ...
    for mode in S9_MODES:
        ...
        directional = [
            flag
            for flag in s9.flags
            if flag.entry_mode is mode
            and flag.reason == "directional_winner_concentration"
        ]
...
def test_markdown_states_the_window_trend_and_every_split(s9) -> None:
    ...
    for cell in s9.cells:
        assert (
            f"| {cell.entry_mode.value} | {cell.split_kind} | {cell.split_key} |" in markdown
        )
