"""Typed client for the execution-service HTTP surface.

Extracted from session-hedging/src/execution.py. Two contracts here are
deliberate and must survive any refactor:

- Transport failures **raise nothing but return UNKNOWN**, never REJECTED. The
  order may or may not have reached the broker, so the caller must reconcile
  rather than resubmit. This is the opposite of ta-notify's never-raise
  contract, and for the opposite reason: a silently dropped order submission is
  the worst failure mode available here.
- ``OPERATION_NAMESPACE`` is fixed forever. Operation ids are derived from it,
  so changing it makes every in-flight operation look new to the gateway and
  opens a second position where a retry was intended.

The settings dependency is a Protocol rather than a concrete Settings class, so
any service can supply its own configuration object.
"""
...
class ExecutionClient:
    ...
    def __init__(self, settings: SupportsExecution, client: httpx.AsyncClient) -> None:
        self._s = settings
        self._client = client
