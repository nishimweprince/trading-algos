...
A bad session name, unknown notification channel, or malformed session spec fails at startup
with a message on stderr and exit code 1. Confirm the timeframe by checking that a
`signal_fired` record's `bucket_start` lands on a `:00 / :05 / :10` boundary.
