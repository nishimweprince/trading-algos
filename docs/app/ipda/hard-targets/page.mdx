...
gold's ~$40 dollar risk would require `1.00` BTC lot (full-coin notional), which a
10K account typically cannot margin. Shipped `volume` is `0.05` (~$2 risk) so orders
fit alongside XAUUSD; raise only when free margin allows. Volumes above mt5-trader
`MAXIMUM_VOLUME` are rejected with `volume_cap_exceeded` (HTTP 422).
