...
`market`. Optional: `stop_loss`, `take_profit`, `expires_at`, `deviation_points`, `note`,
`ignore_signal_age`. Unknown fields are rejected (`extra="forbid"`), and timestamps must be timezone-aware.
...
| `source` | `trading_central` \| `autochartist` | Signal provider; written to the MT5 order comment for attribution. |
