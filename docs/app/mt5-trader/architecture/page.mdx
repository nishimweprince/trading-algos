...
7. **Build request.** `_build_request` maps `(execution_type, direction)` to MT5 order-type
   ...
   number and a broker comment equal to `source` (`trading_central`, `autochartist`, or
   `lux_algo`). Distance-based SL/TP are resolved against the execution reference price before
   send.
...
| `NOTIFICATIONS_ENABLED` | `false` | After each new terminal signal outcome, POST to [notification-service](/notification-service). |
...
of the same `signal_id` do not re-notify.
