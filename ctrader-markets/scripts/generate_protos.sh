#!/usr/bin/env bash
# Regenerate src/ctrader/_generated/ from the vendored proto/ schemas.
#
...
for generated in "$OUT"/*_pb2.py; do
  perl -pi -e 's/^import (OpenApi\w+_pb2) as /from ctrader._generated import $1 as /' \
    "$generated"
done
