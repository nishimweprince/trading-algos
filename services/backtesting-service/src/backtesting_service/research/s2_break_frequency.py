...
from .episodes import Episode, build_episodes, tercile_edges, tercile_label
...
def run_s2_break_frequency(
    ...
    *,
    ...
    ratios = [episode.orb_range_pips / episode.atr_pips for episode in episodes if episode.atr_pips]
...
def _walk(episode: Episode, forward: list[Candle], orb_close: datetime, horizon: float) -> _Break:
...
def _cells(records: list[S2Episode]) -> list[S2Cell]:
    ...
    for group_kind, group_key, subset in _groups(records):
        for horizon in S2_HORIZON_HOURS:
            ...
            opposite_hours = [
                item.opposite_break_hours for item in rows if item.opposite_break_hours is not None
            ]
            cells.append(
                S2Cell(
...
def _s2_cells_section(report: S2BreakFrequencyReport) -> list[str]:
    ...
    return ["## Break classes, every group and horizon", ""] + markdown.table(
        [
            ...
            "CI high",
            "No-break rate",
            "Median first break h",
            "Median opposite break h",
        ],
        rows,
        align_right_from=2,
    )
