...
from ..cell_stats import pair_cost_r, pair_gross_r, pair_outcome
...
def _winner_side(result: TradePairResult) -> str | None:
    ...
    legs = [leg for leg in (result.primary, result.hedge, *result.unknown_legs) if leg is not None]
...
def _splits(structures: list[_Structure]) -> list[tuple[str, str, list[_Structure]]]:
    ...
    for half in ("first", "second"):
        splits.append(("calendar_half", half, [item for item in structures if item.half == half]))
...
def _cell(
    ...
    return S9RegimeCell(
        ...
        gross_expectancy_r=(sum(item.gross_r for item in subset) / len(subset) if subset else None),
...
def _flags(mode: EntryMode, cells: list[S9RegimeCell]) -> list[S9DirectionalFlag]:
    flags: list[S9DirectionalFlag] = []
    overall = next(
        (cell for cell in cells if cell.entry_mode is mode and cell.split_kind == "all"),
        None,
    )
    ...
    halves = [
        cell for cell in cells if cell.entry_mode is mode and cell.split_kind == "calendar_half"
    ]
    ...
    regimes = [
        cell for cell in cells if cell.entry_mode is mode and cell.split_kind == "trend_regime"
    ]
