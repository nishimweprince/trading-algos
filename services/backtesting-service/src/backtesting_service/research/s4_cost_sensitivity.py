...
def _s4_summary(report: S4CostSensitivityReport) -> list[str]:
    ...
    for mode in report.entry_modes:
        ...
        zero_cost = next(
            (cell for cell in cells if cell.configured_execution_cost_pips_per_side == 0.0),
            None,
        )
    ...
    lines += markdown.table(
        [
            ...
            "Cells",
            "Net R at zero cost",
            "Cells net-pips positive",
            "Highest cost/side still net-pips positive",
            f"Cells meeting {report.headroom_gate:g}x",
            "Best headroom",
        ],
        rows,
    )
