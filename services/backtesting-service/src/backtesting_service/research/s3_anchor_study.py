...
def _cell(
    ...
    *,
    ...
    stats = next((row for row in report.session_anchor_stats if row.session == anchor.name), None)
...
def _s3_reading(report: S3AnchorStudyReport) -> list[str]:
    ...
    if incumbent is None:
        ...
        return lines
    better = [cell for cell in ny if not cell.is_incumbent and cell.net_r > incumbent.net_r]
    lines += [
