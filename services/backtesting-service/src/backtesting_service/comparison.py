...
def compare_entry_modes(
    ...
    *,
    ...
    for mode in COMPARISON_MODES:
        ...
        runs[mode] = _ModeRun(row=_comparison_row(engine, report), report=report, engine=engine)
...
def _comparison_row(engine: ClosedBarEngine, report: BacktestReport) -> EntryModeComparisonRow:
...
def _attribution(hedge: _ModeRun, synthetic: _ModeRun) -> HedgeSyntheticAttribution:
    hedge_gap_pips, hedge_gap_r, hedge_gap_count = _tagged_gross(hedge, tag="gap")
    synth_gap_pips, synth_gap_r, synth_gap_count = _tagged_gross(synthetic, tag="gap")
    hedge_same_pips, hedge_same_r, hedge_same_count = _tagged_gross(hedge, tag="same_bar")
    synth_same_pips, synth_same_r, synth_same_count = _tagged_gross(synthetic, tag="same_bar")
    ...
    return HedgeSyntheticAttribution(
        ...
        reconciliation_error_pips=net_pips - (payoff_pips + gap_pips + same_pips - total_cost),
        ...
        reconciliation_error_r=net_r - (payoff_r + gap_r + same_r - total_cost_r),
