...
from backtesting_service.research.s5_resolver_bias import (
    S5_EXECUTABLE_TIERS,
    render_s5_markdown,
...
EXPORTS = [
    FIXTURES / f"session-hedging-XAUUSD-{timeframe}.csv" for timeframe in ("M15", "H1", "H4")
]
...
def test_changed_structures_and_paired_deltas_are_complete(report) -> None:
    ...
    for tier in report["tiers"][:4]:
        ...
        assert tier["net_r"] - baseline["net_r"] == pytest.approx(tier["delta_vs_tier_0"]["net_r"])
