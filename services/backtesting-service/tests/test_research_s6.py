...
from backtesting_service.research.s6_walk_forward import render_s6_markdown, run_s6_walk_forward
...
def test_fold_grid_is_complete_and_immediately_following(report) -> None:
    ...
    for fold in report["folds"]:
        ...
        assert all(
            evaluations[item]["data_role"] == "training" for item in fold["training_evaluation_ids"]
        )
...
def test_all_evaluations_are_logged_and_cscv_is_complete(report) -> None:
    ...
    assert (
        len({item["evaluation_id"] for item in report["evaluations"]}) == report["evaluation_count"]
    )
