...
from backtesting_service.firm_profile import FirmProfile
from backtesting_service.models import Candle, EngineParams, Timeframe
from backtesting_service.risk_guards import PropGuard
...
def test_floating_drawdown_trips_guard_without_closed_loss() -> None:
    ...
    assert (
        engine._open_pair("new_york", 89, 1, entry_ts + timedelta(minutes=30), bullish=True)
        is False
    )
