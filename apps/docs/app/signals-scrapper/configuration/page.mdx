...
Forwarding to the [execution-service (MT5 adapter)](/execution-service) uses a **two-switch safety model**: this bot's
...
| Variable | Default | Purpose |
|----------|---------|---------|
| `MT5_SIGNAL_TRADING_ENABLED` | `false` | Master switch for forwarding signals to the execution-service (MT5 adapter). |
| `MT5_SIGNAL_API_URL` | `http://127.0.0.1:8000` | Execution-service (MT5 adapter) base URL (loopback). |
