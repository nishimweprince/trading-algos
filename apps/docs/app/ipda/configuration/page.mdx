...
| `MT5_SIGNAL_API_URL`, `MT5_SIGNAL_API_KEY`, `REQUIRE_READY` | execution-service (MT5 adapter) connection |
...
By default `DATA_API_URL` targets **the execution-service (MT5 adapter)'s `GET /v1/market-data/candles`**, which returns:
...
Set `DATA_API_KEY` to the same value as the execution-service (MT5 adapter)'s `API_KEY` — that endpoint requires
`X-API-Key` auth. `QUOTE` must be one of the adapter's `ALLOWED_SYMBOLS`.
