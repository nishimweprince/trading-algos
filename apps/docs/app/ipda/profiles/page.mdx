...
A single ipda process can research and trade **multiple quotes** within one profile, and you can
run forex and Deriv side by side from one clone via named profiles.
...
# or
cp symbols.example.deriv.json symbols.deriv.json   # Vol indices, Crash/Boom, Step
```
...
Strategy, session, and notification settings remain **profile-level**. Only execution and risk
calibration fields are per-instrument.
...
| `STOP_LOSS_PIPS` / `TAKE_PROFIT_PIPS` | 40 / 50 | **50 / 80** (wider — see [Hard Targets](/ipda/hard-targets)) |
...
`TRADING_SESSIONS=` empty. See [Sessions](/ipda/sessions).

### Pairing checklist

| ipda variable | Must match |
|---------------|------------|
| `MT5_SIGNAL_API_URL`, `DATA_API_URL` host/port | execution-service (MT5 adapter) `PORT` for that profile |
| `MT5_SIGNAL_API_KEY`, `DATA_API_KEY` | execution-service (MT5 adapter) `API_KEY` for that profile |
