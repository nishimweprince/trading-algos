...
class MassiveDTOTransformer(BaseDTOTransformer):
    """Transforms Massive API (Polygon.io) responses to normalized DTOs"""
    
    def transform_candles(self, raw_response: dict) -> List[CandleDTO]:
        """Transform Massive API aggregate response to List[CandleDTO]"""
        candles = []
        try:
            # Log raw response for debugging
            logger.debug(f"Raw response keys: {raw_response.keys()}")
            logger.debug(f"Response status: {raw_response.get('status')}")
            logger.debug(f"Response resultsCount: {raw_response.get('resultsCount', 0)}")
            ...
            for item in results:
                # Massive API format: {t: timestamp_ms, o: open, h: high, l: low, c: close, v: volume, vw: vwap, n: transactions}
                timestamp_ms = item.get('t', 0)
                # Convert milliseconds to datetime (fromtimestamp expects seconds)
                timestamp = pd.Timestamp.fromtimestamp(timestamp_ms / 1000.0)
            ...
            logger.debug(f"Transformed {len(candles)} candles")
        except Exception as e:
            logger.error(f"Error transforming candles: {e}")
    ...
    def transform_price(self, raw_response: dict) -> PriceDTO:
        """Transform Massive API last quote response to PriceDTO"""
        try:
            # Massive API format: {status: 'success', last: {bid: float, ask: float, exchange: int, timestamp: ms}}
            ...
            timestamp_ms = last.get('timestamp', 0)
            timestamp = pd.Timestamp.fromtimestamp(timestamp_ms / 1000.0) if timestamp_ms else pd.Timestamp.now()
    ...
    def transform_account(self, raw_response: dict) -> AccountDTO:
        """Massive API is data-only, does not support account operations"""
        raise NotImplementedError("Massive API does not support account operations (data-only API)")
    
    def transform_order(self, raw_response: dict) -> OrderDTO:
        """Massive API is data-only, does not support order operations"""
        raise NotImplementedError("Massive API does not support order operations (data-only API)")
    
    def transform_trade(self, raw_response: dict) -> TradeDTO:
        """Massive API is data-only, does not support trade operations"""
        raise NotImplementedError("Massive API does not support trade operations (data-only API)")
    
    def transform_trades_list(self, raw_response: dict) -> List[TradeDTO]:
        """Massive API is data-only, does not support trades list operations"""
        raise NotImplementedError("Massive API does not support trades list operations (data-only API)")
