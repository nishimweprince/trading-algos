...
class InstrumentMapper:
    ...
    # Mapping from standard format (EUR_USD) to Massive API tickers
    ...
    @classmethod
    def to_massive_ticker(cls, instrument: str) -> str:
        """Convert standard format (EUR_USD) to Massive API ticker (C:EURUSD)"""
        # Check if already a Massive ticker
        if instrument.startswith('C:'):
            return instrument
        
        # Try direct mapping
        if instrument in cls.MASSIVE_TICKER_MAP:
            return cls.MASSIVE_TICKER_MAP[instrument]
        
        # Try to construct ticker from instrument name
        # Format: EUR_USD -> C:EURUSD
        try:
            base, quote = instrument.split('_')
            ticker = f"C:{base}{quote}"
            logger.info(f"Mapped {instrument} to {ticker} (constructed)")
            return ticker
        except ValueError:
            logger.warning(f"Could not parse instrument format: {instrument}")
            return instrument
    ...
    @classmethod
    def from_massive_ticker(cls, ticker: str) -> str:
        """Convert Massive API ticker to standard format"""
        # Check if already standard format
        if '_' in ticker and not ticker.startswith('C:'):
            return ticker
        
        # Try to parse ticker format: C:EURUSD -> EUR_USD
        try:
            if ticker.startswith('C:'):
                # Extract currency pair
                pair = ticker[2:]  # Remove 'C:'
                # Split into base and quote (assuming 3-letter codes)
                if len(pair) == 6:
                    base = pair[:3]
                    quote = pair[3:]
                    standard = f"{base}_{quote}"
                    logger.info(f"Mapped {ticker} to {standard} (parsed)")
                    return standard
        except Exception as e:
            logger.warning(f"Could not parse Massive ticker format: {ticker} - {e}")
